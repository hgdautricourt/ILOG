...
export function moveLaserPointer(laserPointer: HTMLSpanElement, x: number, y: number) {
    ...
    requestAnimationFrame(() => {
        ...
        // Calculate the new position of the laser pointer using the acceleration values
        const newLeft = Math.max(0, Math.min(x + (window.innerWidth /2), window.innerWidth - POINTER_SIZE));
        const newTop = Math.max(0, Math.min(y + (window.innerWidth / 2), window.innerHeight - POINTER_SIZE));
